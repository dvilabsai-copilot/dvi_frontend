...
const QUOTE_ID = process.env.E2E_MUNNAR_QUOTE_ID?.trim() ?? '';

// AxisRooms auth key
const AUTH_KEY = process.env.E2E_AXISROOMS_AUTH_KEY?.trim() ?? '';
...
test.describe('AxisRooms MUNNAR QUEEN hotel details integration', () => {
  ...
  test.beforeAll(async ({ request }) => {
    test.skip(!QUOTE_ID, 'Set E2E_MUNNAR_QUOTE_ID for the external sandbox fixture.');
    test.skip(!AUTH_KEY, 'Set E2E_AXISROOMS_AUTH_KEY outside source control for the external sandbox.');

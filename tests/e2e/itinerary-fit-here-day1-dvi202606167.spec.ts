...
const RESET_BASIC_INFO_PAYLOAD = {
  plan: {
    itinerary_plan_id: 9706,
    agent_id: 126,
    staff_id: 0,
    location_id: 0,
    arrival_point: "Madurai Airport",
    departure_point: "Trivandrum, Domestic Airport",
    itinerary_preference: 2,
    itinerary_type: 2,
    preferred_hotel_category: [],
    hotel_facilities: [],
    trip_start_date: "2026-07-01T16:00:00+05:30",
    trip_end_date: "2026-07-04T12:00:00+05:30",
    pick_up_date_and_time: "2026-07-01T16:00:00+05:30",
    arrival_type: 1,
    departure_type: 1,
    no_of_nights: 3,
    no_of_days: 4,
  ...
  routes: [
    {
      ...
      itinerary_route_date: "2026-07-01T00:00:00+05:30",
      no_of_days: 1,
      no_of_km: 12.4,
    ...
    },
    {
      ...
      itinerary_route_date: "2026-07-02T00:00:00+05:30",
      no_of_days: 2,
      no_of_km: 173,
      direct_to_next_visiting_place: 0,
      via_route: "",
    ...
    },
    {
      ...
      itinerary_route_date: "2026-07-03T00:00:00+05:30",
      no_of_days: 3,
      no_of_km: 308,
      direct_to_next_visiting_place: 0,
      via_route: "",
    ...
    },
    {
      ...
      next_visiting_location: "Trivandrum, Domestic Airport",
      itinerary_route_date: "2026-07-04T00:00:00+05:30",
      no_of_days: 4,
      no_of_km: 89.4,
      direct_to_next_visiting_place: 0,
      via_route: "",
...
test("day 2 chained Fit Here flow adds Ramanatha, then Agni, then Gandhamadhana and reports the final Gandhamadhana state", async ({
  ...
  const resetPlanId = 9706;

  await resetItineraryBasicInfo(request, token);
  await deleteManualHotspotIfPresent(request, token, resetPlanId, RAMANATHA_SWAMI_TEMPLE_ID);
  await deleteManualHotspotIfPresent(request, token, resetPlanId, AGNI_TEERTHAM_ID);
  await deleteManualHotspotIfPresent(request, token, resetPlanId, GANDHAMADHANA_PARVATHAM_ID);
  ...
  const rebuildResponse = await request.post(`${API_BASE_URL}/itineraries/${resetPlanId}/route/${dayTwoRouteId}/rebuild`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  expect(rebuildResponse.ok(), "Day 2 route rebuild should succeed before the chained fit test").toBeTruthy();

  const rebuiltDetails = await fetchItineraryDetails(request, token, QUOTE_ID);
  const rebuiltDayTwo = (rebuiltDetails.days || []).find((day: any) => Number(day?.dayNumber || 0) === 2);
  expect(Number(rebuiltDayTwo?.id || 0)).toBe(dayTwoRouteId);
  ...
  for (const step of chainSteps) {
    const previewResponse = await request.post(`${API_BASE_URL}/itineraries/${resetPlanId}/manual-hotspot/fit-preview`, {
      headers: { Authorization: `Bearer ${token}` },
      data: {
    ...
    if (previewJson?.canConfirm === true) {
      const acknowledgedRemovedHotspotIds = Array.isArray(previewJson?.removedHotspots)
        ...
        : [];
      const confirmResponse = await request.post(`${API_BASE_URL}/itineraries/${resetPlanId}/manual-hotspot/fit-confirm`, {
        headers: { Authorization: `Bearer ${token}` },
        data: {

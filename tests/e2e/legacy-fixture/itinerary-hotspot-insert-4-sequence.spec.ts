...
async function getInsertedHotspotIds(
  ...
  const days = Array.isArray(body.days) ? body.days.map(asRecord) : [];
  const targetDay = days.find((day) => Number(day.id) === TARGET_ROUTE_ID);
  const segments = Array.isArray(targetDay?.segments) ? targetDay.segments.map(asRecord) : [];
  return segments
     .filter((segment) => segment.type === 'attraction' && Number(segment.hotspotId) > 0)
     .map((segment) => Number(segment.hotspotId));
}
...
async function previewHotspot(
  ...
  return { ok: res.ok(), status: res.status(), body: asRecord(await res.json().catch(() => ({}))) };
}

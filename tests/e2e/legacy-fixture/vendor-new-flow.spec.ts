...
const EDIT_VENDOR_ID = process.env.E2E_VENDOR_EDIT_ID ? Number(process.env.E2E_VENDOR_EDIT_ID) : null;
...
test('edit existing vendor and persist data across all tabs', async ({ page, baseURL }) => {
  const editVendorId = EDIT_VENDOR_ID;
  if (!Number.isInteger(editVendorId) || editVendorId <= 0) throw new Error('Legacy vendor edit requires E2E_VENDOR_EDIT_ID; no fixed record fallback is permitted');
  ...
  await page.goto(`${baseURL}/vendor/${editVendorId}`, { waitUntil: 'domcontentloaded' });
  await maybeLogin(page);

  if (!page.url().includes(`/vendor/${editVendorId}`)) {
    await page.goto(`${baseURL}/vendor/${editVendorId}`, { waitUntil: 'domcontentloaded' });
  }

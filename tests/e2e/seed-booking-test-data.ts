...
const BASE_URL = process.env.E2E_FRONTEND_BASE_URL!;
...
async function createScenarioQuote(
  ...
  if (!res.ok) {
    const body = await res.text();
    console.log('[seed] create scenario failed', input.label, res.status, body.slice(0, 300));
    return null;
  }
...
async function forceScenario2Quote(token: string): Promise<string> {
  ...
  for (let attempt = 1; attempt <= 12; attempt++) {
    ...
    await updateRouteTimes(token, created.planId, routeId, '13:00:00', '23:00:00');

    const source = await getRouteSourceCoords(routeId);
    if (!source) continue;

    const nearHotel = await pickHotelIdByDistance(source, 'near', attempt - 1);
    if (!nearHotel) {
...
async function forceScenario3Quote(token: string): Promise<string> {
  ...
  for (let attempt = 1; attempt <= 9; attempt++) {
    ...
    const farHotel = await pickHotelIdByDistance(source, 'far', attempt - 1);
...
function writeEnvE2E(values: Record<string, string>, mustVisitHotspotId?: number | null): string {
  ...
  const updates: Record<string, string> = {
    E2E_FRONTEND_BASE_URL: BASE_URL,
  ...
  if (mustVisitHotspotId && mustVisitHotspotId > 0) {
    updates.E2E_BOOKING_RULE_EXPECTED_MUST_VISIT_FIRST_HOTSPOT_ID = String(mustVisitHotspotId);
  }
  ...
  if (!lines.some((line) => line.trim() === '# Auto-generated by tests/e2e/seed-booking-test-data.ts')) {
    lines.unshift('# Auto-generated by tests/e2e/seed-booking-test-data.ts');
  }
...
async function main() {
  ...
  const forceGeneratedScenarios = new Set<ScenarioKey>([
    'afterNoonSameCityWithin20',
    'afterNoonSameCityBeyond20',
    'guideTotal',
  ]);
  const seedableScenarios = (Object.keys(SCENARIO_ENV) as ScenarioKey[]).filter(
    (scenario) => !forceGeneratedScenarios.has(scenario),
  );
  const missing = seedableScenarios.filter((k) => !selected[k]);
  if (missing.length) {
    ...
    const stillMissing = seedableScenarios.filter((k) => !selected[k]);
    if (stillMissing.length) {
      ...
      const unresolved = seedableScenarios.filter((k) => !selected[k]);

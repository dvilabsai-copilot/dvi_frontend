...
- Transitional controller `ItineraryDetailsController.tsx`: 14,089 lines; it is explicitly documented as the remaining staging module to split by workflow.
...
- Extracted boundaries now include loader/error, confirmed-quote banner, media/share/source, delete confirmation, all-hotspots preview, clipboard, para recommendations, day header, timeline segments, guide modal/delete workflows, activity availability/preview workflows, wallet balance/panel/top-up workflow, route switching, hotel selection/search/voucher workflows, hotel refresh/rebuild, selection coverage, hotspot deletion/matrix recovery/preview/priority approval/Fit Here preview/dialog/error/confirmation reset/normalization helpers, computed totals, and the previously documented state/derived boundaries.

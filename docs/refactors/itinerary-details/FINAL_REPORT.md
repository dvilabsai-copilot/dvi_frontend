...
- Transitional controller `ItineraryDetailsController.tsx`: 862 physical lines; it is now below the transitional 1,000-line target.
...
- New in this checkpoint: selected-attraction best-insertion-slot and requested-slot fallback presentation are isolated in `HotspotBestInsertionSlotPanel.tsx`.
- New in this checkpoint: inserted-hotspot matrix route-leg metrics and distance comparison messaging are isolated in `HotspotPreviewRouteSummary.tsx`.
- New in this checkpoint: inserted-hotspot status badges and route/timing insight presentation are isolated in `HotspotPreviewInsertedStatus.tsx`.
- New in this checkpoint: priority-reschedule/P3-removal confirmation presentation is isolated in `HotspotPriorityConfirmation.tsx`.
- New in this checkpoint: attraction priority, visit-time, duration, and timing badges are isolated in `HotspotPreviewAttractionMeta.tsx`.
- New in this checkpoint: conflict override/timing warning heading presentation is isolated in `HotspotConflictNotice.tsx`.
- New in this checkpoint: conflicting hotspot arrival, stay, departure, and onward-travel timing analysis is isolated in `HotspotConflictTimingDetails.tsx`.
- New in this checkpoint: quotation external-stay, non-TBO, and TBO prebook hotel review sections are isolated in `QuotationHotelReviewSections.tsx`.
- New in this checkpoint: quotation summary/wallet/rooming/passenger/loading notice composition is isolated in `QuotationConfirmationOverview.tsx`, with insufficient-wallet controls in `QuotationWalletInsufficientPanel.tsx`.
- New in this checkpoint: assigned-guide summary, cost, edit, and delete presentation is isolated in `ItineraryDayGuideCard.tsx`.
- New in this checkpoint: daily guide/header/segment composition is isolated in `ItineraryDaysSection.tsx`.
- New in this checkpoint: hotel-list loading and vehicle-unavailable page states are isolated in `HotelListLoadingState.tsx` and `VehicleUnavailableState.tsx`.
- New in this checkpoint: hotel-list scroll-anchor, pagination, selection, voucher, and refresh callback composition is isolated in `ItineraryHotelListSection.tsx`.
- New in this checkpoint: an unreachable legacy activity-preview JSX branch was removed from the controller after prior activity-dialog extraction.
- New in this checkpoint: the add-hotspot dialog's city tabs, loading state, and available-hotspot card column are isolated in `HotspotDialogListColumn.tsx`.
- New in this checkpoint: matrix-missing retry guidance and no-feasible-slot insertion warnings are isolated in `HotspotMatrixMissingNotice.tsx` and `HotspotMatrixNoFeasibleNotice.tsx`.
- New in this checkpoint: bounded manual-optimizer attempt diagnostics are isolated in `HotspotManualAttemptLog.tsx`.
- New in this checkpoint: selected preview strategy, insertion-slot decision, optimizer diagnostics, anchor-fit insight, and matrix retry/no-feasible notices are isolated in `HotspotPreviewStrategyPanel.tsx`.
- New in this checkpoint: candidate insertion-slot cards, route-fit metrics, distance comparisons, and decision-reason labels are isolated in `HotspotInsertionSlotsPanel.tsx`.
- New in this checkpoint: effective preview segment rows, timing overrides, travel metrics, attraction/conflict details, insertion-slot evidence, and priority confirmation presentation are isolated in `HotspotPreviewTimelineRows.tsx`.
- New in this checkpoint: preview-fit failures, removed/unscheduled hotspot diagnostics, attempted-anchor messaging, and matrix rebuild action are isolated in `HotspotPreviewValidationNotice.tsx`.
- New in this checkpoint: the right-side proposed timeline, Fit Here selection, preview notices/rows, and apply action are isolated in `HotspotPreviewPane.tsx`.
- New in this checkpoint: shared travel-label endpoint parsing is isolated in `hotspotText.utils.ts`; post-loader callbacks and optional cost-breakdown runtime guards are stabilized.
- New in this checkpoint: route-family base-quote normalization is isolated in `routeFamily.utils.ts`.
- New in this checkpoint: quotation confirmation dialog composition is isolated in `QuotationConfirmationDialog.tsx` while state and submission ownership remain unchanged.
- New in this checkpoint: itinerary-level voucher, pluck-card, invoice, incidental-expense, cancellation, and hotel-voucher composition is isolated in `ItineraryAncillaryModals.tsx`.
- New in this checkpoint: manual and automatic Fit Here preview dialog composition is isolated in `ItineraryFitHereDialogs.tsx`.
- New in this checkpoint: gallery, video, clipboard, source-preview, email-share, and all-hotspots-preview dialog composition is isolated in `ItineraryMediaDialogs.tsx`.
- New in this checkpoint: hotel search and room-selection dialog composition is isolated in `ItineraryHotelDialogs.tsx`.
- New in this checkpoint: hotspot preview timeline, matrix/slot decisions, city/list derivations, and modal scroll effects are isolated in `useHotspotPreviewViewModel.ts`.
- New in this checkpoint: selected-hotel, vehicle, entry-ticket, financial-total, hydrated-day, and display-day derivation is composed in `useItineraryCostViewModel.ts`.
- New in this checkpoint: Add Hotspot dialog shell and two-column child composition is isolated in `ItineraryAddHotspotDialog.tsx`.
- New in this checkpoint: previous-day hotel billing confirmation callbacks and route-time continuation are isolated in `useArrivalPolicyDecisionDialog.ts`.
- New in this checkpoint: route-time/rebuild progress overlay presentation is isolated in `ItineraryRouteProgressOverlay.tsx`.
- New in this checkpoint: quotation confirmation amount, passenger summary, occupancy preview, and field-helper derivation are isolated in `useQuotationConfirmationViewModel.ts`.
- New in this checkpoint: Fit Here progress interval lifecycle and loading-step updates are isolated in `useFitHereProgressTimer.ts`.
- New in this checkpoint: obsolete hotel-search DTO/filter scaffolding was removed after the active hotel workflow extraction.
- New in this checkpoint: obsolete commented hotel DTO duplication was removed from the controller.
- New in this checkpoint: route-option de-duplication/sorting and route-family base-quote fallback derivation are isolated in `useItineraryRouteOptionsViewModel.ts`.
- New in this checkpoint: header, daily itinerary, hotel/vehicle sections, package/cost summary, and action controls are composed in `ItineraryDetailsTravelSections.tsx`.
- New in this checkpoint: hotspot deletion, activity add/delete, and guide assignment/delete dialogs are composed in `ItineraryActivityGuideDialogs.tsx`.
- New in this checkpoint: manual/automatic Fit Here dialog prop shaping and reset callbacks are isolated in `useFitHereDialogProps.ts`.
- New in this checkpoint: hotel-search and room-selection dialog prop shaping and reset behavior are isolated in `useItineraryHotelDialogProps.ts`.
- New in this checkpoint: gallery, video, clipboard, source-preview, share, and all-hotspots-preview prop shaping are isolated in `useItineraryMediaDialogProps.ts`.
- New in this checkpoint: quotation confirmation overview/review/passenger/travel/footer prop assembly is isolated in `useItineraryQuotationDialogProps.ts`.
- New in this checkpoint: voucher, pluck-card, invoice, incidental-expense, cancellation, and hotel-voucher prop assembly are isolated in `useItineraryAncillaryModalProps.ts`.
- New in this checkpoint: copy-link, WhatsApp-share, and email-share actions are isolated in `useItineraryShareActions.ts`.
- New in this checkpoint: hotspot deletion, route rebuild, route-time patching, and arrival-policy continuation are isolated in `useItineraryRouteMutationWorkflow.ts`.
- New in this checkpoint: quotation dialog child/infant counts are null-safe during the initial itinerary loading render.
- New in this checkpoint: activity availability, preview/add/delete, guide availability, guide assignment, and guide modal orchestration are isolated in `useItineraryActivityGuideWorkflow.ts`.
- New in this checkpoint: hotspot preview selection, reset state, route-day lookup, filtering, and preview view-model composition are isolated in `useItineraryHotspotPreviewWorkflow.ts`.
- New in this checkpoint: clipboard recommendation defaults, hotel-rate visibility, and active hotel-group initialization are isolated in `useItineraryClipboardSelectionWorkflow.ts`.
- New in this checkpoint: loaded itinerary page markup and child dialog mounting are isolated in `ItineraryDetailsPageView.tsx`.
- New in this checkpoint: post-refresh scrolling, Fit Here scroll restoration, and route-progress cleanup are isolated in `useItineraryScrollEffects.ts`.
- New in this checkpoint: hotel-row route/stay identity deduplication is isolated in `hotelRows.utils.ts`.
- New in this checkpoint: hotspot conflict presentation and Add Hotspot dialog prop composition are isolated in `useItineraryHotspotDialogWorkflow.ts`.
- New in this checkpoint: quotation confirmation dialog prop assembly is isolated in `useItineraryQuotationDialogWorkflow.ts`.
- New in this checkpoint: media, clipboard, source-preview, and all-hotspots dialog prop assembly is isolated in `useItineraryMediaDialogWorkflow.ts`.
- New in this checkpoint: arrival-policy, Fit Here, hotel-search, and ancillary dialog prop assembly is isolated in `useItinerarySupportingDialogWorkflow.ts`.
- New in this checkpoint: stable hotel-save, mounted, fetch, and route-switch refs are isolated in `useItineraryPageRefs.ts`.
- New in this checkpoint: hotel/vehicle totals, scroll anchors, selection guards, and hotel pagination wiring are isolated in `useItineraryHotelPageWorkflow.ts`.
- New in this checkpoint: page-loader stage updates and route-time progress wiring are isolated in `useItineraryRouteProgressWorkflow.ts`.
- New in this checkpoint: source-preview actions and route hotel-detail cache wiring are isolated in `useItineraryRouteSupportWorkflow.ts`.
- New in this checkpoint: route-option loading, family cache reset, prefetching, and switching are isolated in `useItineraryRouteOptionsWorkflow.ts`.
- New in this checkpoint: quotation passenger counts remain zero-safe while itinerary data is `null` during loading.
- New in this checkpoint: clipboard selection, content rendering, and copy actions are isolated in `useItineraryClipboardWorkflow.ts`.
- New in this checkpoint: early-arrival marker hydration and page loading/navigation guards are isolated from the controller.
- New in this checkpoint: unused legacy controller imports and destructured declarations were removed without changing the router/runtime/controller export boundary.
- New in this checkpoint: hotspot force-conflict gating, validation blocking, and apply-button label derivation are isolated in `useHotspotApplyPresentation.ts`.

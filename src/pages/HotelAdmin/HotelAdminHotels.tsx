...
import {
  listHotelAdminHotels as listHotels,
  updateHotelAdminHotel as updateHotel,
  deleteHotelAdminHotel as deleteHotel,
...
const HotelPage: React.FC = () => {
  ...
  const handleView = (row: HotelRow) => {
    navigate(`/hotel-admin/hotels/${row.backendId}/preview`);
  };
  const handleEdit = (row: HotelRow) => {
    navigate(`/hotel-admin/hotels/${row.backendId}/edit`);
  };
  ...
  const goRoomsPriceBook = () => {
    ...
    navigate("/hotel-admin/rates");
  };
  const goAmenitiesPriceBook = () => {
    setPbOpen(false);

    const storedHotelId = Number(
      window.localStorage.getItem(
        "hotelAdminSelectedHotelId",
      ) || 0,
    );

    const storedHotel = rows.find(
      (row) =>
        Number(row.backendId) ===
        storedHotelId,
    );

    const hotelId = Number(
      storedHotel?.backendId ??
        rows[0]?.backendId ??
        0,
    );
    ...
    if (!hotelId) {
      window.alert(
        "No assigned hotel is available for Price Book.",
      );
    ...
    navigate(
      `/hotel-admin/hotels/${hotelId}/pricebook`,
    );
  };
        ...
        <div className="hotel-card-head">
          ...
          <div className="hotel-head-actions">
            ...
            <button
              ...
              onClick={() => navigate("/hotel-admin/hotels/new")}
            >
              + Add Hotel
            </button>
